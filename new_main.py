...
class IntegratedQASystem:
    ...
    def _fetch_recent_history(self, session_id: str) -> list:
        ...
        获取最近5轮对话历史
        """
        # todo 通过session_id，也就是本次对话的唯一标识，拿取近n轮对话历史。作用就是给模型做为历史对话使用。
        #  对话历史表已经在初始化时创建，不用单独创建。创建方法 init_conversation_table
        try:
    ...
    def update_session_history(self, session_id: str, question: str, answer: str) -> list:
        ...
        try:
            # 插入新的对话记录
            # todo 将对话中的信息：session_id, question, answer, 加上时间戳，一起存入表中。
            #   如果系统比较完整的话，还要存储用户id。
            #   时间戳的作用：
            #   1. 确保对话历史按时间顺序排列，方便后续查询
            #   2. 任何插入到mysql的数据，都要写时间戳。create_time，update_time，没有就会被认为是小白。
            self.mysql_client.cursor.execute("""
    ...
    def query(self, query, source_filter=None, session_id=None):
        ...
        接受用户问题，返回答案
        :param query: 用户问题
        :param source_filter: 分类过滤
        :param session_id: 唯一标识。
        :return:
        """
        """查询集成系统，支持对话历史和流式输出"""
        ...
        # todo 历史对话，这里是session_id下的5轮对话，如果没有就返回空。
        history = self.get_session_history(session_id) if session_id else []
        # 执行 BM25 搜索，获取答案和是否需要 RAG 的标志
        # todo 去FQA中查询相似问题的答案，并根据答案判断是否走rag。
        #  need_rag true和false，比如缓存中没有答案，或者小于阈值，或者整体报错了，就走rag
        answer, need_rag = self.bm25_search.search(query, threshold=0.85)
        if answer:
            # 如果找到可靠答案，记录答案到日志
            self.logger.info(f"MySQL答案: {answer}")
            # todo 每次有答案，就将问题和答案，存储到对应的session_id下面。
            #  不只是作为对话历史使用。作为一个需要存储的日志也要存储，比如之后可以分析用户的行为，或者分析回答的是否合理。
            if session_id:
                # 更新对话历史
            ...
            collected_answer = ""
            # 从 RAG 系统获取流式输出
            # todo 如果need_rag是True，就进入rag部分，流式输出。
...
def main():
    ...
    # 打印支持的知识分类
    print(f"支持的知识分类：{qa_system.config.VALID_SOURCES}")
    # 提示用户输入查询或退出
    ...
    try:
        while True:
            ...
            if query.lower() == "exit":
                ...
                break
            # 获取用户输入的分类过滤
            source_filter = input(f"请输入知识分类 ({'/'.join(qa_system.config.VALID_SOURCES)}) (直接回车默认不过滤): ").strip()
            if source_filter and source_filter not in qa_system.config.VALID_SOURCES:
                # 如果分类过滤无效，记录警告日志
                logger.warning(f"无效的知识分类 '{source_filter}'，将不过滤")
                # 设置为空，忽略过滤
                source_filter = None
            # 打印答案提示
            print("\n答案: ", end="", flush=True)
            # 初始化累积答案的字符串
            answer = ""
            # todo 前边模拟用户输入，拿到用户的问题 ，还有过滤条件，还有一个uuid生成的本次对话的唯一标识。输入到qa_system.query。
            # 迭代 query 方法的生成器
