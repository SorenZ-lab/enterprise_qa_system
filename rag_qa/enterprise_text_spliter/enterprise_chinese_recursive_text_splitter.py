...
# 1. 切分的分隔符。中英文不一样，我们自己定义的分隔符。
# 2. 切分后分隔符是否保留。在本方法中，保留到到每个块的后边。 英文的是三种选择，不保留，保留在文本块之前后，保留在文本块之前。
# 3. 如果分隔符最后一个不是"", chunk_size还有没有用。比如一块的长度是100token，没有""这个分隔符，即使设置了chunk_size，不会起作用。
