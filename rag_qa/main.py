...
def main(query_mode=True, directory_path="data"):
    ...
    # 根据模式执行不同操作
    # todo 离线部分包括两个主要方法，一个是加载数据和切分数据，输出子块信息，一个是将子块存入向量库。
    if not query_mode:
        # --- 数据处理模式 ---
        logger.info("进入数据处理模式...")
        total_chunks_added = 0
        # ["product", "tech", "hr", "admin"]
        for source_dir in conf.VALID_SOURCES:
            ...
            if os.path.exists(dir_path):
                ...
                try:
                    # 加载数据 + 切分
                    # 子块的内容 + 子块元数据（子块id，父块id，父块内容，文档位置，分类，时间）
        ...
        while True:
            ...
            source_filter_input = input(f"请输入知识分类 ({'/'.join(valid_sources)}) (直接回车默认不过滤)：").strip()
            source_filter = None # 默认不过滤
            if source_filter_input:
                if source_filter_input in valid_sources:
                    source_filter = source_filter_input
                    logger.info(f"用户选择了分类过滤: {source_filter}")
                else:
                    logger.warning(
                        f"无效的知识分类 '{source_filter_input}'，将不过滤"
                    )
                    print(f"提示：输入的分类 '{source_filter_input}' 无效，将不过滤。")
...
if __name__ == "__main__":
    ...
    # 或者通过命令行参数控制
    # import argparse
    # parser = argparse.ArgumentParser(description="Enterprise QA System Main Entry Point")
