...
class RAGSystem:
    ...
    def generate_answer(self, query, source_filter=None):
        ...
        if context_docs:
            ...
            logger.info(f"检索出来的文本块：{context_docs}")
        ...
        # Todo 6. 将rag中检索的所有的文本信息 和用户query一起构建好提示词模板，给到大模型生成并返回答案。
        try:
            answer = self.llm(prompt_input)
        except Exception as e:
            logger.error(f"调用 LLM 生成最终答案失败: {e}")
            answer = f"抱歉，处理您的企业咨询问题时出错。请联系人工客服：{conf.CUSTOMER_SERVICE_PHONE}"
